    ...
    <div className=" px-10 max-w-[100vw] mx-auto mt-8 p-3 sm:p-8 bg-bgSecondary border border-primary text-gray-100 shadow-xl rounded-xl">
      <div className="container mx-auto my-10 px-6 md:px-12 lg:px-20 text-center mb-12  p-3 sm:p-8 rounded-lg shadow-lg">
        ...
        <p className="mt-4 text-base md:text-lg text-gray-300 animate-fadeInUp delay-200">
          Express your thoughts, share your stories, and connect with a
      ...
      <form
        ...
        className="space-y-4 border border-bgPrimary rounded-xl p-5"
      >
        ...
        <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
          ...
          <div className="flex space-x-2">
            ...
            />
            <Button label="Add Tag" type="button" onClick={handleAddTag} />
          </div>
        </div>
          ...
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
            <div className="flex flex-wrap gap-2">
              {blogTags.map((tag, index) => (
                <div
                  key={index}
                  className="bg-gray-700 text-gray-200 py-1 px-3 rounded-md flex items-center space-x-2"
                >
                  <span>{tag}</span>
                  <button
                    type="button"
      ...
      {preview && (
        <div className="mt-6 p-4 border border-gray-600 rounded-md bg-gray-800 shadow-lg">
          ...
          <h1 className="text-4xl my-5 font-extrabold px-10 capitalize text-primary">
            {title}
          </h1>
          <div
            dangerouslySetInnerHTML={{ __html: preview }}
            className="blog-container max-h-[20rem] overflow-y-scroll"
          />
        </div>
      )}
    </div>
