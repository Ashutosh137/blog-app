...
function BlogsGroup({ BlogPost }: { BlogPost: Blog[] }) {
    ...
    const [Sort, setSort] = useState<"newest" | "like" | "oldest" | "default" | string>("default");
    ...
    useEffect(() => {
        ...
        if (Sort === "newest") {
            sortedBlogs = sortedBlogs.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
        } else if (Sort === "oldest") {
    ...
    return (
        <div className="flex mx-auto  mt-10 p-2 flex-col">
            <div className="flex w-full space-x-5 px-5">
                ...
                <select className='text-base w-full max-w-40 font-medium text-black capitalize p-2 border border-primary rounded-md ' name="Sort" value={Sort} onChange={(e) => setSort(e.target.value)}>
            ...
            <p className="text-gray-700 text-lg text-center  mt-5">***---Nothing Else---***</p>
        </div>
    );
}
